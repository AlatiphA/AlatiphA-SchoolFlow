...
/* ---------- view switching ---------- */
const views = ['home', 'setup', 'staff', 'classes', 'students', 'subjects', 'grades', 'remarks', 'reports', 'history'];
...
function renderQuickAccessList() {
  ...
  wrap.innerHTML = QUICK_ACCESS_CARDS.map(c => `
  ...
  firebase.auth().onAuthStateChanged(user => {
    if (user) {
      ...
      if (localStorage.getItem(GUEST_MODE_KEY)) {
        hideAuthGate(); hideSchoolChoiceGate(); hidePendingGate();
        initLockScreen();
        if (!appStarted) { appStarted = true; proceedToApp(); }
      } else {
        hideSchoolChoiceGate(); hidePendingGate();
        renderAuthForm();
        showAuthGate();
      }
    }
