// AlatiphA SchoolFlow — app-6.js
const APP_VERSION = 'v6.3';

// Uploaded photos/signatures can arrive in whatever format the source
// app/browser happens to hand back (JPEG, WEBP, etc.) — jsPDF's
// addImage() only reliably embeds PNG/JPEG, and a format it can't
// decode (e.g. WEBP, which some phones/browsers supply from the
// gallery/file picker while others supply JPEG for the same photo)
// fails silently, so the image just never appears on the printed
// report card. Every uploaded image is redrawn onto a canvas and
// re-encoded as PNG once, here, at upload time, so what actually gets
// stored — and later handed to jsPDF — is always a clean, decodable
// PNG no matter what the original file was or which browser supplied it.
...
let currentStatus = null; // 'active' | 'pending' | 'suspended' | 'rejected'
let teacherAssignedClassIds = [];
let teacherAssignedSubjectIds = [];
...
const QUICK_ACCESS_CARDS = [
  { view: 'setup', title: 'Setup', description: 'School info, term, and report layout', teacherHidden: true,
  ...
  { view: 'manage-teachers', title: 'Manage Teachers', description: 'Approve, assign classes, suspend', headteacherOnly: true,
    icon: '<circle cx="9" cy="7" r="4"/><path d="M2 21v-2a5 5 0 0 1 5-5h4a5 5 0 0 1 5 5v2"/><path d="M17 8l3 3-3 3"/><path d="M20 11h-6"/>' },
  { view: 'staff', title: 'Staff', description: 'Staff records, ranks, and signatures', teacherHidden: true,
  ...
  { view: 'classes', title: 'Classes', description: 'Create and manage your classes', teacherHidden: true,
  ...
  { view: 'subjects', title: 'Subjects', description: 'Manage the subjects taught', teacherHidden: true,
  ...
  { view: 'history', title: 'Term History', description: 'Browse and export past terms', teacherHidden: true,
...
function renderQuickAccessList() {
  ...
  const isScopedTeacher = FIREBASE_ENABLED && currentRole === 'teacher';
  const cards = QUICK_ACCESS_CARDS.filter(c =>
    (!c.headteacherOnly || currentRole === 'headteacher') && (!c.teacherHidden || !isScopedTeacher)
  );
...
function renderHome() {
  ...
  const dashEl = document.getElementById('teacherDashboard');
...
// Fetches this teacher's own assignment fresh from their account record
// (not the local school cache) — assignments are managed by the Head
// Teacher and can change, so this always reflects the current truth.
function renderTeacherDashboard() {
  const dashEl = document.getElementById('teacherDashboard');
  dashEl.innerHTML = '<p class="hint">Loading your assignment…</p>';
  firebase.firestore().collection('users').doc(currentUid).get().then(doc => {
    const data = doc.exists ? doc.data() : {};
    const assignedClassIds = data.assignedClassIds || [];
    const assignedSubjectIds = data.assignedSubjectIds || [];
    const classes = DB.get(KEYS.classes, []).filter(c => assignedClassIds.includes(c.id));
    const subjects = DB.get(KEYS.subjects, []).filter(s => assignedSubjectIds.includes(s.id));

    document.getElementById('welcomeSubtext').textContent = classes.length
      ? `You're assigned to ${classes.map(c => c.name).join(', ')}.`
      : "You haven't been assigned any classes yet — check with your Head Teacher.";

    const classList = classes.length
      ? classes.map(c => `<span class="td-chip">${escapeHtml(c.name)}</span>`).join('')
      : '<span class="td-chip td-chip-empty">None assigned yet</span>';
    const subjectList = subjects.length
      ? subjects.map(s => `<span class="td-chip">${escapeHtml(s.name)}</span>`).join('')
      : '<span class="td-chip td-chip-empty">None assigned yet</span>';

    dashEl.innerHTML = `
      <div class="td-section">
        <div class="td-label">Class</div>
        <div class="td-chips">${classList}</div>
      </div>
      <div class="td-section">
        <div class="td-label">Subjects</div>
        <div class="td-chips">${subjectList}</div>
      </div>
      <div class="td-actions">
        <button type="button" class="btn-primary td-action" data-view="students">Students</button>
        <button type="button" class="btn-primary td-action" data-view="grades">Enter Scores</button>
        <button type="button" class="btn-primary td-action" data-view="reports">My Reports</button>
      </div>
    `;
    dashEl.querySelectorAll('.td-action').forEach(btn => {
      btn.addEventListener('click', () => showView(btn.dataset.view));
    });
  }).catch(err => {
    dashEl.innerHTML = `<p class="hint">Could not load your assignment: ${escapeHtml(err.message)}</p>`;
  });
}
...
document.getElementById('schoolLogo').addEventListener('change', e => {
  ...
  if (!file) return;
  normalizeImageFile(file, 400).then(dataUrl => {
    const s = DB.get(KEYS.settings, {});
    s.logo = dataUrl;
    DB.set(KEYS.settings, s);
    loadSettingsForm();
  }).catch(() => alert('Could not read that image. Please try a different file.'));
});
...
function fillClassSelect(sel) {
  let classes = DB.get(KEYS.classes, []);
  if (FIREBASE_ENABLED && currentRole === 'teacher') {
    classes = classes.filter(c => teacherAssignedClassIds.includes(c.id));
  }
  ...
  list.querySelectorAll('.edit-staff-signature-input').forEach(input => {
    input.addEventListener('change', e => {
      ...
      normalizeImageFile(file, 400).then(dataUrl => {
        ...
        if (st) st.signature = dataUrl;
        DB.set(KEYS.staff, staffList);
        renderStaff(); // stays in edit mode — editingStaffId is untouched
      }).catch(() => alert('Could not read that signature image. Please try a different file.'));
    });
  });
...
document.getElementById('addStaffBtn').addEventListener('click', () => {
  ...
  if (file) {
    normalizeImageFile(file, 400).then(commit).catch(() => {
      commit('');
      alert('Could not read that signature image; staff member saved without one.');
    });
  } else {
    commit('');
  }
...
document.getElementById('syncNowBtn').addEventListener('click', () => {
  ...
  const refreshAssignment = currentRole === 'teacher'
    ? firebase.firestore().collection('users').doc(currentUid).get().then(doc => {
        const data = doc.exists ? doc.data() : {};
        teacherAssignedClassIds = data.assignedClassIds || [];
        teacherAssignedSubjectIds = data.assignedSubjectIds || [];
      })
    : Promise.resolve();
  Promise.all([pullCloudData(), refreshAssignment]).then(() => {
...
function showSuspendedGate() { document.documentElement.classList.add('suspendedAccess'); }
function hideSuspendedGate() { document.documentElement.classList.remove('suspendedAccess'); }
function showRejectedGate() { document.documentElement.classList.add('rejectedAccess'); }
function hideRejectedGate() { document.documentElement.classList.remove('rejectedAccess'); }

// Lets someone whose join request was rejected clear that link and
// try again with a different school's code, rather than being stuck.
function clearSchoolLinkAndRetry() {
...
function renderManageTeachers() {
  ...
  fetchSchoolMembers().then(members => {
    ...
    members.forEach(m => {
      ...
      const statusBadge = m.status === 'pending' ? ' · Pending approval'
        : m.status === 'suspended' ? ' · Suspended'
        : m.status === 'rejected' ? ' · Rejected'
        : '';
      const assignedClassNames = (m.assignedClassIds || [])
        .map(id => { const c = classes.find(x => x.id === id); return c ? c.name : null; })
        .filter(Boolean);
      const assignedSubjectNames = (m.assignedSubjectIds || [])
        .map(id => { const s = subjects.find(x => x.id === id); return s ? s.name : null; })
        .filter(Boolean);
      const assignedText = (assignedClassNames.length || assignedSubjectNames.length)
        ? ` · Classes: ${assignedClassNames.join(', ') || '—'} · Subjects: ${assignedSubjectNames.join(', ') || '—'}`
        : '';
      ...
      if (m.status === 'pending') {
        ...
        ).join('') || '<p class="hint">No classes created yet.</p>';
        const subjectCheckboxes = subjects.map(s =>
          `<label class="checkbox-row"><input type="checkbox" class="assign-subject-cb" value="${s.id}"> ${escapeHtml(s.name)}</label>`
        ).join('') || '<p class="hint">No subjects created yet.</p>';
        li.innerHTML = `<div class="edit-row">
          <strong>${escapeHtml(m.email || m.uid)}</strong>
          <div class="meta">${roleLabel}${statusBadge}</div>
          <p class="hint">Assign classes:</p>
          ${classCheckboxes}
          <p class="hint">Assign subjects:</p>
          ${subjectCheckboxes}
          <div class="edit-actions">
        ...
        li.innerHTML = `<div><strong>${escapeHtml(m.email || m.uid)}</strong>
            <div class="meta">${roleLabel}${statusBadge}</div></div>
          <div class="actions">
            <button class="edit-student reconsider-teacher-btn" data-uid="${m.uid}">Reconsider</button>
          </div>`;
      } else if (editingTeacherUid === m.uid) {
        // Editing an already-active (or suspended) teacher's assignment —
        // same checkbox picker as approval, pre-checked with what they
        // currently have, so a Head Teacher can add/remove classes and
        // subjects at any time, not just once at approval.
        const classCheckboxes = classes.map(c =>
          `<label class="checkbox-row"><input type="checkbox" class="assign-class-cb" value="${c.id}" ${(m.assignedClassIds || []).includes(c.id) ? 'checked' : ''}> ${escapeHtml(c.name)}</label>`
        ).join('') || '<p class="hint">No classes created yet.</p>';
        const subjectCheckboxes = subjects.map(s =>
          `<label class="checkbox-row"><input type="checkbox" class="assign-subject-cb" value="${s.id}" ${(m.assignedSubjectIds || []).includes(s.id) ? 'checked' : ''}> ${escapeHtml(s.name)}</label>`
        ).join('') || '<p class="hint">No subjects created yet.</p>';
        li.innerHTML = `<div class="edit-row">
          <strong>${escapeHtml(m.email || m.uid)}</strong>
          <div class="meta">${roleLabel}${statusBadge}</div>
          <p class="hint">Classes:</p>
          ${classCheckboxes}
          <p class="hint">Subjects:</p>
          ${subjectCheckboxes}
          <div class="edit-actions">
            <button class="save-btn save-assignment-btn" data-uid="${m.uid}">Save</button>
            <button class="cancel-btn cancel-assignment-btn">Cancel</button>
          </div>
        </div>`;
      } else {
        li.innerHTML = `<div><strong>${escapeHtml(m.email || m.uid)}</strong>
          ...
          <div class="actions">
            ${isSelf || m.role === 'headteacher' ? '' : `
              <button class="edit-class edit-assignment-btn" data-uid="${m.uid}">Edit</button>
              ${m.status === 'suspended'
                ? `<button class="edit-student reactivate-teacher-btn" data-uid="${m.uid}">Reactivate</button>`
                : `<button class="del-student suspend-teacher-btn" data-uid="${m.uid}">Suspend</button>`}
            `}
          </div>`;
      }
      list.appendChild(li);
    ...
    list.querySelectorAll('.approve-teacher-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        firebase.firestore().collection('users').doc(btn.dataset.uid)
          .update({ status: 'active', assignedClassIds, assignedSubjectIds })
    ...
    list.querySelectorAll('.reject-teacher-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        if (!confirm('Reject this request? You can reconsider it later if needed.')) return;
        firebase.firestore().collection('users').doc(btn.dataset.uid)
    ...
    list.querySelectorAll('.reconsider-teacher-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        firebase.firestore().collection('users').doc(btn.dataset.uid)
          .update({ status: 'pending' })
          .then(() => renderManageTeachers())
          .catch(err => alert('Could not reconsider: ' + err.message));
      });
    });
    list.querySelectorAll('.suspend-teacher-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        if (!confirm('Suspend this teacher? They will be blocked from opening the app until reactivated. Their data stays in the school.')) return;
        firebase.firestore().collection('users').doc(btn.dataset.uid)
          .update({ status: 'suspended' })
          .then(() => renderManageTeachers())
          .catch(err => alert('Could not suspend: ' + err.message));
      });
    });
    ...
    list.querySelectorAll('.edit-assignment-btn').forEach(btn => {
      btn.addEventListener('click', () => { editingTeacherUid = btn.dataset.uid; renderManageTeachers(); });
    });
    list.querySelectorAll('.cancel-assignment-btn').forEach(btn => {
      btn.addEventListener('click', () => { editingTeacherUid = null; renderManageTeachers(); });
    });
    list.querySelectorAll('.save-assignment-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        const li = btn.closest('li');
        const assignedClassIds = Array.from(li.querySelectorAll('.assign-class-cb:checked')).map(cb => cb.value);
        const assignedSubjectIds = Array.from(li.querySelectorAll('.assign-subject-cb:checked')).map(cb => cb.value);
        firebase.firestore().collection('users').doc(btn.dataset.uid)
          .update({ assignedClassIds, assignedSubjectIds })
          .then(() => { editingTeacherUid = null; renderManageTeachers(); })
...
function initAuth() {
  ...
  document.getElementById('suspendedLogoutBtn').addEventListener('click', () => firebase.auth().signOut());
  document.getElementById('rejectedLogoutBtn').addEventListener('click', () => firebase.auth().signOut());
  document.getElementById('rejectedRetryBtn').addEventListener('click', clearSchoolLinkAndRetry);
  ...
  document.getElementById('registerSchoolBtn').addEventListener('click', () => {
    ...
    const ok = confirm(
      `Create "${name}" as a brand-new school with you as Head Teacher?\n\n` +
      `If your school already exists in this app, use "Join with a Code" instead and ask your Head Teacher for the code — do NOT register again, or you'll end up with two separate, disconnected schools.`
    );
    if (!ok) return;
    ...
    if (user) {
      ...
      firebase.firestore().collection('users').doc(currentUid).get().then(userDoc => {
        ...
        if (!data || !data.schoolId) {
          currentSchoolId = null; currentRole = null; currentStatus = null;
          hideSyncingMessage(); hideAuthGate(); hideSuspendedGate(); hideRejectedGate();
          showSchoolChoiceGate();
          return;
        }
        ...
        if (data.status === 'rejected') {
          currentSchoolId = null; currentRole = data.role; currentStatus = 'rejected';
          hideSyncingMessage(); hideAuthGate(); hideSuspendedGate(); hidePendingGate();
          showRejectedGate();
          return;
        }
        if (data.status !== 'active') {
          // Suspended, or any other unexpected status — never treat as
          // active by default. This is what a Head Teacher suspending a
          // teacher actually blocks.
          currentSchoolId = null; currentRole = data.role; currentStatus = data.status || 'suspended';
          hideSyncingMessage(); hideAuthGate(); hidePendingGate(); hideRejectedGate();
          showSuspendedGate();
          return;
        }
        ...
        return pullCloudData().then(() => {
          hideSyncingMessage(); hideAuthGate(); hidePendingGate(); hideSuspendedGate(); hideRejectedGate();
          initLockScreen();
          if (!appStarted) { appStarted = true; proceedToApp(); }
      ...
      if (localStorage.getItem(GUEST_MODE_KEY)) {
        hideAuthGate(); hideSchoolChoiceGate(); hidePendingGate(); hideSuspendedGate(); hideRejectedGate();
        initLockScreen();
        if (!appStarted) { appStarted = true; proceedToApp(); }
      } else {
        hideSchoolChoiceGate(); hidePendingGate(); hideSuspendedGate(); hideRejectedGate();
        renderAuthForm();
        showAuthGate();
      }
    }
